...
public class Mario implements Runnable{
    ...
    private BufferedImage show=null;//状态对应图像
    private BackGround backGround=new BackGround();//获取障碍物的信息
    private Thread thread=null;//实现马里奥的动作
    ...
    public void run() {
        while(true){
            ...
            for (int i = 0; i < backGround.getObstacleList().size(); i++) {
                ...
                //判断马里奥是否在障碍物上
                if ((ob.getX() > this.x - 30 && ob.getX() < this.x + 25) && ob.getY() == this.y + 25) {
                    onObstacle = true;
                }
                //判断是否跳起来顶到方块
